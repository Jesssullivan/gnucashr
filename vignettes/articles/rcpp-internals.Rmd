...
```
This article documents the Rcpp implementation details in gnucashr. It is
intended for contributors who want to understand, modify, or extend the
C++ code.
...
GnuCash stores monetary values as fractions to avoid floating-point errors.
These functions provide vectorized conversion with exact arithmetic.
...
Converts GnuCash fraction representation to double. This is a hot path in
...
Adds fractions using exact integer arithmetic with LCM-based common denominator.
Returns reduced fractions.
...
The `thread_local` keyword ensures each thread gets its own RNG instance,
preventing data races in parallel code.
...
`generate_guids(n)` generates multiple GUIDs efficiently by reusing the same
RNG instance.

## Monte Carlo Simulation (monte-carlo.cpp)

The Monte Carlo module uses RcppParallel for embarrassingly parallel simulation.
...
- Different grain boundaries produce different results (expected with parallelism)
...
Use `RMatrix<double>` and `RVector<double>` for thread-safe concurrent writes:
...
The linear-to-2D index conversion allows parallel iteration over the grid.

## Validation (validation.cpp)

Transaction validation uses exact integer arithmetic to verify double-entry
balance.
...
2. **Avoid R API calls** inside worker `operator()` - no `Rf_*`, no `Rcpp::*`
