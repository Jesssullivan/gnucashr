---
title: "Importing Data from Payment Platforms"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Importing Data from Payment Platforms}
...
gnucashr provides standardized importers for common payment platforms and accounting
systems. These importers convert platform-specific CSV exports into a common format
that can be easily analyzed or imported into GnuCash.
...
All importers return a tibble with the `gnucashr_csv_import` class containing
these standardized columns:
...
Some importers include additional columns like `fee` for transaction fees.
...
PayPal exports vary slightly depending on account type and region. gnucashr
auto-detects the format and maps these columns:
...
Stripe exports amounts in the smallest currency unit (cents for USD). The importer
automatically detects this and converts to standard decimal format:
...
2. Customize date range and columns
3. Export to CSV

**QuickBooks Desktop:**
1. Go to Reports > Accountant & Taxes > Transaction List by Date
2. Set date range
...
- **Desktop format** - QuickBooks Desktop (has separate Debit/Credit columns)
...
Use `combine_csv_imports()` to merge transactions from multiple sources:
...
When importing from multiple sources, you may have duplicate transactions
(e.g., a PayPal payment that also appears in QuickBooks). Use the `deduplicate`
option:
...
Use `validate_csv_import()` to check for common issues:
```{r validate}
# Check for issues
...
Use `preview_csv_mapping()` to debug import issues:
...
- How each column maps to the standard format
...
- Generate financial reports with `trial_balance()` and `income_statement()`
- Perform cash flow analysis with the forecasting tools

See the [Getting Started](getting-started.html) vignette for more on working
with GnuCash data in R.
